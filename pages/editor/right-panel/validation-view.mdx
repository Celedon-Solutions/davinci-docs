...
Tests can also report evaluation-criteria configuration errors here. Numeric criteria that contain an incomplete OR union (for example, `< -20 or`), an invalid clause, or any overlap between a union clause and another verdict target cannot determine a verdict. Complete the malformed clause or adjust the boundaries so each verdict owns a distinct region.
...
When clicked, the button opens the [Davinci Agent](/pages/editor/right-panel/agent-chat) chat panel and automatically adds a message referencing the TBD object with a prompt like "Please help resolve TBD fields for this attribute." This approach is used for TBDs because they often require additional context to resolve properly - the agent needs to know what value should replace the TBD, what units to use, or what equation logic to apply.
...
[Read More About Merge Conflicts](/pages/editor/right-panel/version-control#merge-conflicts)
