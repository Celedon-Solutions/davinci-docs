...
<Info>
Davinci also includes structured [Planning](/pages/editor/agent/planning), [Skills](/pages/editor/widgets/skills-rules), and [Model Review](/pages/editor/agent/model-review) workflows for guiding how the agent plans, executes, and inspects engineering work.
</Info>
...
<Note>
[Skills](/pages/editor/widgets/skills-rules) guide how the agent uses available tools. They do not add new tools by themselves.
</Note>
...
### Simulation
Tools for the compiled-simulation route: `SetupSimulation` (mesh a geometry and
validate a boundary-condition program), `CompileSimulation` (build the certified
Solution Space), `QuerySimulation` (QoIs and certificates at parameter points),
`OptimizeSimulation` (certified QoI optimization over the parameter box),
`VerifySimulation` (a full-order truth solve at one point), `CaptureSimulationView`
(headless renders of the setup or a live field) and `GetSimulationReference` (the
closed BC-language reference with worked examples). `RunSimulation` executes Python
simulation code objects.
...
### Reviews
Tools for building, updating, and running structured model reviews. Review tools can compare model areas against each other or compare model content against reference source material.
