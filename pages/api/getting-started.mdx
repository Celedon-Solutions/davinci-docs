...
- **Create and manage projects** – List, create, and delete projects with pagination
- **Open persistent sessions** – Connect to projects with persistent chat history that survives restarts
- **Send messages** – Instruct the agent to perform modeling tasks
- **Receive events** – Get real-time updates with cursor-based polling and event type filtering
- **Reconnect sessions** – Resume inactive sessions with full chat context
...
A **session** represents a persistent connection to a project. Sessions:
- Load the project into the Design Engine
- Enable message passing to the agent
- Provide access to project data
- **Persist metadata and chat history** -- sessions survive server restarts
- Have a 30-minute idle timeout (moves to `inactive`; use `keepalive` to extend)
- Maximum active duration of 24 hours
- Can be **reconnected** after becoming inactive or closed, resuming with full chat context

### Session Lifecycle

```
connected → (idle timeout) → inactive → (reconnect) → connected
connected → (close) → closed → (reconnect) → connected
connected → (DE unavailable) → degraded
```
...
- `session.reconnected` – Session was restored from an inactive/closed state
- `agent.thinking` – Agent has started processing (fires before LLM calls)
- `agent.reply` – Agent has responded
- `agent.error` – An error occurred
- `session.disconnect` – Session ended

Events include a monotonically increasing `eventId`. Use cursor-based polling (`?cursor=<lastEventId>`) to avoid re-processing events. Use `eventTypes` to filter for specific event types.
...
### Sessions
```
GET    /api/v1/programmatic/sessions                          # List all sessions
...
POST   /api/v1/programmatic/sessions/:id/reconnect            # Reconnect inactive session
GET    /api/v1/programmatic/sessions/:id/history              # Get chat history
...
### Files
```
GET    /api/v1/programmatic/sessions/:id/files                      # List files
POST   /api/v1/programmatic/sessions/:id/files/upload               # Upload file
GET    /api/v1/programmatic/sessions/:id/files/download/:fileName   # Download file
DELETE /api/v1/programmatic/sessions/:id/files/:fileId              # Delete file
```

## File Operations

Upload and download files (PDFs, documents, images) to your projects programmatically.
...
```typescript JavaScript
// Download file by UUID filename
const fileBuffer = await session.downloadFile('abc123-uuid.pdf');
...
```python Python
# Download file by UUID filename
file_data = await session.download_file("abc123-uuid.pdf")
