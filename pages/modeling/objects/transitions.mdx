---
title: Transition
icon: /images/objecticons/transition.svg
---
...
<Note>Failure transition types will only activate `toStates` and leave `fromStates` activate.</Note>
...
| [Block](/pages/user-guide/views/block) | Visual block diagram showing transition connections. |
| [History](/pages/editor/model-area) | Track changes to attributes of the object. |

## Properties Fields

<ParamField path="Inheritable" type="boolean">
**Yes.** Transitions can be used as bases or specializations through [object inheritance](/pages/modeling/common/inheritance).
</ParamField>
...
<ParamField path="Conditional" type="string">
  ...
  **Examples:**
  ...
  If empty, the transition always occurs when triggered (for trigger kind) or is always available (for default kind). Transition conditionals use standard syntax with attribute references. See [Equations](/pages/modeling/common/equations) for complete syntax reference.
...
<ParamField path="From State" type="reference">
  ...
  [Read more about States](/pages/modeling/objects/states)
</ParamField>
...
<ParamField path="Constraints" type="object">
  Formal guard rules and validation conditions owned by this transition.

  [Read more about Constraints](/pages/modeling/objects/constraints)
</ParamField>

<ParamField path="Code" type="object">
  Executable implementations owned by this transition for advanced guard or transition behavior.
