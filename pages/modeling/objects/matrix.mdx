---
title: Matrix
icon: /images/objecticons/matrix.svg
---
...
### Value Background Styling

Selected columns can apply a background color when their resolved cell value matches a rule. Add rules from **Styling** in the column settings, then choose:

- A text or number comparison value
- An operator: equals, not equals, contains, or a numeric comparison
- A background color
- Whether the color applies to the matching **cell** or the entire rendered **row**

Rules run in order; the first matching rule in a column wins. When more than one column has a row rule that matches, the leftmost matching column wins. Only non-empty strings and finite numeric values are styled. Empty cells, booleans, arrays, objects, and composite displays such as name-and-value do not apply a rule.
...
**No.** Matrices do not currently support [object inheritance](/pages/modeling/common/inheritance).
</ParamField>
...
<ParamField path="Attributes" type="object">
  Attributes owned by the matrix for query parameters, metadata, and equations associated with the artifact.
