...
Code can be organized beneath packages or owned directly by parts, entities, actions, states, transitions, tasks, requirements, capabilities, and tests. A code object can own Attributes for module-local parameters and calculated values.
...
<Note>Once an Object's value is assigned to a variable, you cannot perform unit conversions on it—conversions must happen at reference time using the `unit` notation.</Note>
...
<Warning>Unit conversions must use compatible units. Attempting to convert incompatable units (such as mass `g` to length `m`) will cause an error.</Warning>
...
| `file.open()` | Load file from model | `file.open(path, mode='rb', encoding='utf-8')` |
...
```python
from davinci import file

# Load text file
content = file.open("@ParentObject/config.txt", mode='r')
print(content)

# Load binary file
binary_data = file.open("@ParentObject/image.png", mode='rb')

# Specify encoding for text files
text = file.open("@ParentObject/data.csv", mode='r', encoding='utf-8')
```

**Function signature:**
```python
file.open(path, mode='rb', encoding='utf-8')
```
...
<Note>State machine functions are primarily used for behavioral simulation. See [States](/pages/modeling/objects/states) and [Transitions](/pages/modeling/objects/transitions) for more information on state machine modeling.</Note>
...
**No.** Code objects do not currently support [object inheritance](/pages/modeling/common/inheritance).
</ParamField>
...
<ParamField path="Attributes" type="object">
  Module-local parameters and calculated values owned by this Code object.
