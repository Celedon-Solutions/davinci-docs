...
Davinci is a collaborative system modeling platform designed to streamline the engineering process. Utilizing state-of-the-art natural language processing, Davinci allows users to generate, modify, and interact with models using simple, intuitive commands. Below are the key features of Davinci.
...
<Note> To see a complete list of modeling objects see: [Modeling Language](/pages/modeling/overview/language) </Note>
...
- **Matrices**: N^2 display of relationships between model objects.
...
- **PDF-Based Generation**: Upload PDF files into Davinci and automatically generate a system model from its contents.
- **Automatic Relationship Generation**: Davinci can determine mechanical, power, data, thermal etc. relationships between parts in your model in seconds.
...
- **Artifact Generation**: Create text, tables, figures, and matrices from model data automatically.

### Collaboration

Davinci supports real-time collaboration, allowing multiple users to work on the same project simultaneously:

- **Real-Time Editing**: See changes from other collaborators as they happen.
- **Object Locking**: Prevent editing conflicts with automatic and manual locking.
- **Project Sharing**: Invite collaborators by email and manage roles and permissions.
- **Version Control**: Create snapshots and branches to track project history.
...
- **Data Export**: Export data in various formats (.sysml, .daf, .json, .docx, .xlsx, .png).
...
- **Service Accounts**: Programmatic API access for automation and CI/CD integration.
...
Davinci uses state-of-the-art models for its design engine and stores project data securely.

For dedicated hosting and custom security solutions, [contact us](https://www.celedon.solutions/enterprise-contact).
